...
- **深色模式** - 支持深色主题
- **转 API** - 将 Token 转换为 Anthropic 兼容 API，支持 Claude Code 等客户端调用

## 转 API 功能

工具支持将 kiro账号以 anthropic API方式提供服务，供claude code, cursor(付费会员)等使用。

转 API是独立功能，内置账号绑定设备 ID 功能(不依赖K-Proxy服务)。 如果只使用转 API，无需开启K-proxy，也无需打开kiro。

**主要特性：**
- 兼容 Anthropic Messages API (`/v1/messages`)
- 支持流式响应和非流式响应
- 多 Token 负载均衡，自动轮询可用 Token
- Token 自动刷新，过期自动续期
- 支持按标签筛选 Token
- 一键配置 Claude Code
...
1. 设置和启动转 API 服务
![设置](images/screenshots/toapi_settings.png)

2. 然后在命令行启动claude开始使用
![claude](images/screenshots/toapi_claude.png)
...
- `POST /v1/messages` - 发送消息（兼容 Anthropic API）
